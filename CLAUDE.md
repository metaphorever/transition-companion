...
**Status: Stages A and B complete. Stage C pending in a separate session.**
**Model: Opus · Effort: high**
**Last session: Phase 15 Stage B — deprecated blocker fields/enums deleted; `BlockersSection.tsx` rewritten around the new structured-blocker schema (resolution_mode form, inline resolution tasks, drill-in breadcrumb via `?trail=`, convert-to-task, confirm-resolve, KB-condition panel, reverse-lookup header); `ItemDetail.tsx` extended with a `BreadcrumbBackLink` reading `?trail=` so navigation chains survive refresh; dashboard reshaped into Active / Working on blockers / Waiting buckets with a unified `BucketItemRow`, plus a presence-gated confirm-resolve nudge surfacing parents whose linked resolution tasks have completed.**

**Phase 15 Stage B carryover notes:**
- **Deprecated fields are gone.** `Blocker` no longer carries `label`, `kb_dependency`, `person_ref`, `user_defined`, `severity`, `resolvable`, `resolvable_note`, `workaround_available`, or `workaround_note`. `BlockerResolvable` and `BlockerSeverity` enums are deleted. `suppress_workaround` remains. Any out-of-tree user export that still has the old shape will pass through the type as excess optional fields are gone — `mergeWithDefaults` doesn't currently strip them on import, so old blockers will keep their stale fields in storage until the user touches them. If that bites in dogfooding, add a one-pass cleanup in storage migration.
...
- **Confirm-resolve has two surfaces.** (1) Always-on prompt at the top of the resolvable-blocker's card on the parent item-detail page. (2) Quiet dashboard nudge listing parent items where any resolution task has flipped to `complete`/`at_risk` — gated on `presence.overall_level !== 'just_the_path' || presence.open_doors`. Just-the-path users only see the prompt when viewing the parent task itself.
- **Dashboard buckets are unified across KB + custom items.** Categorization function lives inline in `Dashboard.tsx` and returns five arrays of `BucketRow`: `activeRows`, `workingRows`, `waitingRows`, `completedRows`, `somedayRows`. Sort: priority (`now` → `soon` → unset → `someday`/`unsure`) then importance. `BucketItemRow` is the shared renderer; `variant: 'active' | 'working' | 'waiting'` tunes the visual treatment without diverging the data shape.
- **Waiting bucket consolidates three concepts**: any active `out_of_control` blocker, `status === 'policy_blocked'`, and `immutable: true`. Renders collapsed by default (reuses the existing `showPolicyBlocked` state variable — the variable name is now slightly misleading but functional). Phase 13's auto-seeded `ssa-marker` and friends land here as expected.
...
- **Custom items now use `category`/`track` from the parent when converted from a blocker.** `BlockersSection` accepts `parentCategory` and `parentTrack` props (passed from `ItemDetail.tsx`). The custom-item path passes `item.category`/`item.track`; the KB-item path passes the same. `convertBlockerToTask` immediately navigates to the spawned task with breadcrumb trail set — the user lands on the new task ready to fill out the next layer.
- **HTML structural fix during verification**: `BlockerCard` originally rendered `<li>` and was wrapped in another `<li>` by the section's list. Browser flagged the nesting; fix was to make `BlockerCard` a `<div>` and let the `<li>` come from the wrapper only. Final live DOM verified with zero nested `<li>`.
- **Verified in browser preview** — dashboard buckets render correctly with seeded test data; blocker card shows mode + status + description + resolution tasks + confirm-resolve prompt; drill-in builds `?trail=parent` URL; back-link pops the trail; reverse-lookup header surfaces on the resolution task showing all parents that reference it.
- **Stage C remaining work**: KB condition change detection (compare `blocker.status_date` vs `condition.status_date` and surface the policy-changed prompt with a "revisit" button — the panel already shows the comparison inline but the Stage A note also calls for a separate Stage C policy-changed pass with Opus copy); optional recurring re-check for personal_circumstance blockers (UI not yet built — schema supports linking a recurring item id, but no wire-up); Opus alert-copy review on every new prompt + the federal-marker-policy `status_summary` and us-marriage-policy text.

**Phase 15 Stage A carryover notes (still relevant):**
...
- **No Node/npm in sandbox** — `npm run build` + `npm test` need to run in the main project directory before deploy or Stage C work begins. Vite dev preview works through the Claude Preview MCP, which is how Stage B verification happened in this session.
...
- **Node/npm not available in sandbox shells (same as Phase 13)** — TypeScript build, vitest test run, and Vite dev preview could not run in the worktree. Verify before deploying by running in the main project directory: `npm run build` (typecheck + Vite build), `npm test` (vitest), `npm run dev` (manual wizard walk-through). Type-correctness was audited by careful inspection: all new types are exported from `src/types/index.ts`; all imports resolve to exported symbols; `tsc --noEmit` confirms locally on the user's machine.
