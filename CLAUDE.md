...
**Last session: Phase 15 Stage C — KB condition change detection finished (the existing per-blocker `KBConditionPanel` now shows a current-status line + reference link, and the policy-changed prompt gained an "I've seen this — keep blocked for now" acknowledgment that bumps `blocker.status_date` to today so the prompt quiets until the next condition change); a presence-gated dashboard nudge (`policyChangedParents` in `Dashboard.tsx`) lists parents whose `kb_condition_ref` advanced past their blocker's `status_date`; personal-circumstance blockers gained an optional recurring re-check reminder — `Blocker` schema grew `reminder_recurring_id?: string`, `BlockerCard` renders a new `PersonalCircumstancePanel` with an inline interval picker (1mo/3mo/6mo/1y) that creates a fixed-mode `RecurringItem` (track inherited from parent, label built from a localizable template), and the linked recurring item surfaces its due/overdue state inline; `addRecurringItem` signature changed to return the new id; `federal-marker-policy` and `us-marriage-policy` `status_summary` texts were pared down from textbook-dense paragraphs to short pointers (reference links carry the substance).**

**Phase 15 Stage C carryover notes:**
- **`addRecurringItem` now returns `string` (the new id)**. Previously `void`. No existing call sites needed updating because `void`-returning calls keep working when the return is widened. New BlockersSection handler `handleAttachReminder` uses the return value to link the new recurring item back to the blocker.
- **Acknowledgment quiets but does not resolve**. Clicking "I've seen this — keep blocked for now" calls `updateBlocker(slug, id, { status_date: today })`. The blocker stays active; the policy-changed prompt simply goes back to sleep until the condition's `status_date` next advances past the new `status_date`. Same machinery on the dashboard nudge — once acknowledged on the item-detail page, the parent stops surfacing in the dashboard nudge list.
- **Re-check reminder UI lives in `BlockerCard` → `PersonalCircumstancePanel`**. Schema: `Blocker.reminder_recurring_id?: string`. Create flow: inline interval picker → creates `RecurringItem` with `mode: 'fixed'`, `interval_days: <chosen>`, `start_date: today`, `track: parentTrack`, label from `blockers.recurring_label_template`. Once linked: shows "Re-check {due/overdue} ({date})" via `dueDateLabel`/`getEffectiveDueDate`, an "I checked in" button when the reminder is due/overdue (calls `logRecurringItem`), an "Open in Recurring" link, and "Remove reminder" (deletes the recurring item AND clears the link in one action). Detach is destructive by design — the recurring item exists *for* this blocker.
...
**Phase 15 cross-stage notes (still load-bearing for Phase 16):**
...
- **KB conditions seeded:** `federal-marker-policy` (covers SSA / passport / passport-card markers) and `us-marriage-policy` (covers us-marriage-cert). Both got pared-down `status_summary` in Stage C. Other immutable items can get `kb_condition_ref` set in later phases as needed.
...
- **HTML structural fix from Stage B**: `BlockerCard` is a `<div>`; the wrapping `<li>` comes from the section's list only. Do not put a `<li>` inside `BlockerCard` itself.
