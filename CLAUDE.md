...
- **Future design — pronouns model and UI**: Current model stores a single string on `profile.pronouns`. User wants: multi-select (not radio), ranked ordering, infinite custom add, an "any pronouns" option, an "any except [exclusions]" option, and a usage preference toggle ("pick one and use it consistently" vs "mix it up" vs other — exact phrasing TBD). This is a schema change (`pronouns: string` → `pronouns: PronounEntry[]` or similar) plus a new UI pattern. Affects onboarding Step 2, Settings profile section, and anywhere pronouns are displayed. Design the storage shape before building.
